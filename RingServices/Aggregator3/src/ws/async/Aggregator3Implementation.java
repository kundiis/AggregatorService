...
import org.example.www.aggregator3.BlogData;
import org.example.www.aggregator3.Search;
import org.example.www.aggregator3.SearchAppendandPass;
import org.example.www.aggregator3.SearchAppendandPassResponse;
...
public class Aggregator3Implementation extends Thread implements Aggregator3SkeletonInterface {
	...
	public SearchResponse search(Search search) {
		...
		saap.setOriginID(3); // id of the 1st WS
		...
		System.out.println("Blog Aggregator Called by client....");
		int i=0;
		// block the request wait for all the WS to reply
		while(i<12)
		{
	...
	public SearchAppendandPassResponse searchAppendandPass(
		...
		StoreClient<String,byte[]> client = factory.getStoreClient("results");
		System.out.println("Blog Aggregator initiated....");
	...
	public SearchGoldBoxResponse searchGoldBox(SearchGoldBox searchGoldBox) {	
		...
		response.setOut(new SearchBlog().searchData(searchGoldBox.getIn()));
		return response;
	}
