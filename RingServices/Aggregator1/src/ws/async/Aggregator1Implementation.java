...
import org.example.www.aggregator1.SearchAppendandPassResponse;
...
public class Aggregator1Implementation extends Thread implements Aggregator1SkeletonInterface {
	...
	public SearchResponse search(Search search) {
		...
		saap.setOriginID(1); // id of the 1st WS
		...
		System.out.println("Image Aggregator Called by client....");
		int i=0;
		...
		while(i<12)
		{
			...
			if(outarray == null)
			{
				...
				for(int j=0;j<st.length;j++)
				{
			        ...
			        if(image.exists())
			        {
			        	System.out.println("Image Exists as "+image.getPath());
	...
	public SearchAppendandPassResponse searchAppendandPass(
		...
		StoreClient<String,byte[]> client = factory.getStoreClient("results");
		System.out.println("Image Aggregator initiated....");
	...
	private void createNewThread(final SearchAppendandPass searchAppendandPass) {
		Thread t1 = new Thread(this){
			...
			// access Voldemort, get Data, append the SOAP and call the next WS
				System.out.println("Thread Started for calling next aggregator...");
	...
	public SearchGoldBoxResponse searchGoldBox(SearchGoldBox searchGoldBox) {	
		...
		response.setOut(new SearchImage().searchData(searchGoldBox.getIn()));
		return response;
	}
